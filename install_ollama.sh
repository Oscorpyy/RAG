...
if [ "$OS" = "Darwin" ]; then
    ...
    if [ ! -L "$HOME/.local/bin/ollama" ] || [ "$(readlink "$HOME/.local/bin/ollama")" != "/Applications/Ollama.app/Contents/Resources/ollama" ]; then
        status "Adding 'ollama' command to PATH (may require password)..."
        mkdir -p "$HOME/.local/bin" 2>/dev/null ||  mkdir -p "$HOME/.local/bin"
        ln -sf "/Applications/Ollama.app/Contents/Resources/ollama" "$HOME/.local/bin/ollama" 2>/dev/null || \
...
download_and_extract() {
  ...
  - Debian/Ubuntu:  apt-get install zstd
  - RHEL/CentOS/Fedora:  dnf install zstd
  - Arch:  pacman -S zstd"
        fi
...
for BINDIR in $HOME/.local/bin /usr/bin /bin; do
