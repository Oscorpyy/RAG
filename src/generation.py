...
Answer generation with Ollama (Optimized & Cleaned Version).
"""
...
def clean_llm_response(text: str) -> str:
    """
    Remove thinking/reasoning blocks (<think>...</think>)
    generated by some models (DeepSeek-R1, Qwen reasoning, etc.).
    """
    if "</think>" in text:
...
def truncate_content(text: str, max_chars: int = MAX_CHARS_PER_SOURCE) -> str:
    """
    Truncate the content of a source to keep the prompt short (and thus fast).
    """
...
def _detect_file_type(file_path: str) -> str:
    """
    Return a human-readable label ("Python Code" / "Markdown Documentation")
...
def format_source_block(
    ...
    """
    Format a source snippet with a clear structure for the LLM:
    - Visible [Source N] tag
    - File type (code vs doc)
    - Content in an appropriate fenced block
    """
    file_type = _detect_file_type(file_path)
    # For Python code, we use a ``` block so the LLM
    # can clearly distinguish code from natural text.
...
def is_ollama_running(host: str = DEFAULT_OLLAMA_HOST) -> bool:
    """
    Check if the Ollama service is active and reachable.
    """
...
def resolve_model(
    ...
    """
    Return requested_model if available in Ollama, otherwise a fallback.
    """
    try:
    ...
    async with semaphore:
        try:
            response = await client.chat(
                ...
                options={
                    "temperature": 0.0,
                    "num_predict": 1024,  # Significantly increased
